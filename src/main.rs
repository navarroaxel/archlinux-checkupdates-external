...
async fn check_chrome_updates() -> Result<(), Error> {
    ...
    let (updates, packages) = join!(
        ...
        fetch_aur_packages(
            products.iter().map(| p | p[0].clone()).collect()
        )
    );
...
async fn check_jetbrains_updates() -> Result<(), Error> {
    let jetbrains_products = vec![
        ...
        vec!["intellij-idea-ue-eap", "IntelliJ IDEA", "IC-IU-EAP-licensing-EAP"],
        vec!["phpstorm", "PhpStorm", "PS-RELEASE-licensing-RELEASE"],
        vec!["phpstorm-eap", "PhpStorm", "PS-EAP-licensing-EAP"],
        vec!["pycharm-community-eap", "PyCharm", "PC-PY-EAP-licensing-EAP"],
        vec!["pycharm-edu", "PyCharm Edu", "PE-RELEASE-licensing-RELEASE"],
        vec!["pycharm-professional", "PyCharm", "PC-PY-RELEASE-licensing-RELEASE"],
    ...
    let (updates, packages) = join!(
        ...
        fetch_aur_packages(
            jetbrains_products.iter().map(| p | p[0].clone()).collect()
        )
    );
...
async fn main() -> Result<(), Error> {
    let (jetbrains_result, chrome_result) = join!(check_jetbrains_updates(), check_chrome_updates());

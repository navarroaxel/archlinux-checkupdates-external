...
mod fetch_tests {
    ...
    #[tokio::test]
    async fn test_fetch_teamviewer_updates_structure() {
        ...
        match fetch_teamviewer_updates().await {
            Ok(updates) => {
                ...
                for update in updates.iter().take(1) {
                    ...
                    // TeamViewer updates should be for teamviewer package
                    assert!(
                        update.name.starts_with("teamviewer"),
                        "Package name should start with 'teamviewer'"
                    );
                }
            }
            Err(e) => {
